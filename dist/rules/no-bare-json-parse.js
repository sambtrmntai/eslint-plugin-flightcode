...
function hasSafeArgument(node) {
    ...
    return (arg.type === AST_NODE_TYPES.CallExpression &&
...
export const noBareJsonParse = createRule({
    name: "no-bare-json-parse",
    meta: {
        type: "problem",
        docs: {
            description: "Require JSON.parse on untrusted input to be guarded by a try/catch (Power-of-10 Rule 5).",
        },
        messages: {
            bareJsonParse: "JSON.parse on untrusted input must be guarded: wrap it in try/catch (or parse a known-safe literal / JSON.stringify output). Unhandled parse errors crash the boundary (Power-of-10 Rule 5).",
        },
        schema: [],
    },
    defaultOptions: [],
