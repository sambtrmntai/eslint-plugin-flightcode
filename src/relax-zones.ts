...
// specific rules over a set of globs (e.g. demo/website/scripts/mocks zones
// that aren't yet burned down to the canonical ruleset). Enforces the
// FlightCode.md "Exemption hierarchy" tier-4 rule: a path zone must not use
// a recursive glob, since that silently swallows every new file added
// underneath it. See FLIGHTCODE.md "Exemption hierarchy".
 ...
 */
function findRecursiveGlobs(globs: string[]): string[] {
 ...
 */
function buildRecursiveGlobErrorMessage(offenders: string[]): string {
    const quoted = offenders.map((glob) => `"${glob}"`).join(", ");
    const plural = offenders.length > 1 ? "globs" : "glob";
    return (
        `relaxZones: recursive ${plural} ${quoted} ${
            offenders.length > 1 ? "are" : "is"
        } not allowed — a path zone must not silently swallow new files. ` +
