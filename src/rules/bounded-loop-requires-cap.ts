 ...
 */
function isDirectDeadlineCall(side: TSESTree.Expression): boolean {
...
function isDeadlineCall(
    ...
    if (
        side.type === AST_NODE_TYPES.BinaryExpression &&
        side.operator === "-"
    ) {
        return (
            isDirectDeadlineCall(side.left) || isDirectDeadlineCall(side.right)
        );
    }
    return false;
}
 ...
 */
function isDeadlinePollComparison(test: TSESTree.Expression): boolean {
    if (test.type !== AST_NODE_TYPES.BinaryExpression) {
        return false;
    }
    if (!CAP_OPERATORS.has(test.operator)) {
 ...
 */
function consequentExitsLoop(consequent: TSESTree.Statement): boolean {
...
function hasCounterCapGuard(
    ...
    return statements.some((stmt) => {
        ...
        return (
            isDeadlinePollComparison(stmt.test) &&
