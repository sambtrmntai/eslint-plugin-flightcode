...
| **`0.1.0`** | superseded (tag `v0.1.0`)  | Introduces rule tiers (Tier 1/2/3), the rule admission tests, and the exemption hierarchy + waiver-registry contract (see sections above), including the explicit-file-list rule for Tier-4 path zones (a zone must be a literal file list — no wildcards at all — so new files can't enter a relaxed area unnoticed). **⚠️ BREAKING:** `relaxZones()` now _enforces_ this — it throws at config-load time if any entry passed to it isn't a literal path (contains `*`, `?`, `[`, `]`, `{`, `}`, or `!`). A consumer whose `eslint.config.js` currently calls `relaxZones()` with any glob (recursive or not, e.g. `demo/**` or even `demo/*.ts`) will fail to load config the moment it bumps its pin to `v0.1.0`; the fix is to rewrite the zone as an explicit file list before bumping, or — for a genuine file-TYPE deviation like `*.cli.ts` — move it to a plain flat-config block instead of `relaxZones()`. No other repo is force-migrated — the pin is the freeze, so a repo stays on its current version until it deliberately moves. `@typescript-eslint/require-await` moves to Tier 3 (`"off"`, explicit rejection — was never curated, arrived only via `recommendedTypeChecked`; see "Rule admission tests" for the worked-example rationale). The five `@typescript-eslint/no-unsafe-*` rules move to Tier 1 (declarative only — already `"error"` via the preset, now also named in the curated block as Pillar 1 / Power-of-10 rule-7 trust-boundary rules; no behavior change). Strict `max-len` was scoped for this release but deliberately deferred pending admission testing (Sam, 2026-07-31) — highland alone would take ~33,309 hits, and it needs to run through the new admission tests first. **Correction (see the `0.2.0` row below): `max-len` ran the admission tests on 2026-08-04 and was REJECTED, not adopted** — this row's original wording implied `0.2.0` would carry it; it does not.                                                                       |
| **`0.2.0`** | **current** (tag `v0.2.0`) | Adds `jsdoc/require-throws` to the curated block as **Tier 1 / normative** (Sam, 2026-08-04) — Pillar 2c names `@throws`/failure modes as a canonical prologue field, and the preset had never turned the corresponding rule back on. Passed all three admission tests: traces to Pillar 2c (test 1); detects exactly the undocumented-throw case, covered by no other Tier-1 rule (test 2); measured 76 hits in highland, sampled ones real omissions — e.g. `demo/lib/assert.ts -> nn()` throws on its failure path with no `@throws` tag anywhere in its prologue (test 3). Two related rules were considered and NOT adopted this release: `jsdoc/require-description` is **REJECTED** — its violations are largely constructors leaning on an adjacent class doc, which is defensible, so it fails admission test 3 (conformance, not defect). `jsdoc/require-file-overview` is right in principle but **queued, not enabled** — deferred behind an in-flight highland change. Separately, correcting the record on strict `max-len`: it was scoped as a candidate for this release and **REJECTED, not adopted** — it fails admission test 2 (Prettier's `printWidth: 80` already enforces the 80-col cap on code structure; `max-len` only re-flags content Prettier deliberately never touches — strings, template literals, comments, regexes — plus atomic tokens it cannot split, so it has no third detection bucket beyond what Prettier + those atomic-token cases already cover) and test 3 (of 265 sampled real violations, **264 were conformance, not defects** — TSDoc prose 44.9%, mandated BDD Given/When/Then test names 33.2%, long strings 12.5%, CSS-in-JS/HTML, trailing comments and regexes — only **one** genuine defect in 265; a maximally-permissive configured variant leaves just 3 hits across 904 files, all long identifiers, so it passes test 3 only vacuously). `max-len` is not currently planned for a future version pending a materially different proposal. |
...
- Failure modes are part of the prologue, not optional prose: `jsdoc/require-throws`
  (**Tier 1 / normative**, added in `0.2.0`) enforces the `@throws` field named above —
  a function that throws must say so in its prologue, or the contract is silently
  incomplete on exactly the field this pillar names.
