...
3. **File-type** — keyed on what the file _is_ (e.g. a `*.cli.ts` glob
   override).
4. **Path zone** — keyed on where the file _lives_ (a directory-glob relax
   block via `relaxZones()`). Last resort; must carry a named retirement
   trigger — the condition under which the zone is revisited and, ideally,
   removed. **Also: a path zone must not use a recursive glob, and this is
   enforced, not just documented.** `relaxZones()` throws at config-load time
   on any glob containing a `**` segment — globs like `demo/**`, `website/**`,
   or `**/scripts/**` would silently swallow every file added under them
   from now on, so the zone would grow without anyone deciding it should,
   and new code would be born non-compliant by default. Write the zone
   non-recursively (`demo/*.ts`) or as an explicit file list, so adding a
   new file to a relaxed area is a deliberate act that shows up in a diff.
   New code must be FlightCode-compliant by default — an exemption you can
   fall into without noticing is not an exemption, it is a hole. There is no
   opt-out flag by design: a consumer who genuinely needs a recursive
   relaxation can hand-write a flat-config block directly and own that
   decision explicitly, rather than routing it through the helper that
   exists specifically to prevent it. (This pairs with the retirement-trigger
   requirement above; together they are what stops a path zone from becoming
   permanent.)
    > **Limitation, stated honestly.** The enforcement lives in the
    > `relaxZones()` helper only. A consumer can still hand-write a flat-config
    > block with a recursive glob and bypass it entirely — nothing in this
    > package stops that. The helper makes the easy path the correct path; it
    > is not a hard guarantee against a determined bypass.
