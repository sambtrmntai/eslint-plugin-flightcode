...
4. **Path zone** — keyed on where the file _lives_ (a directory-glob relax
   ...
   removed. **Also: a path zone must not use a recursive glob.** Globs like
   `demo/**`, `website/**`, or `**/scripts/**` silently swallow every file
   added under them from now on, so the zone grows without anyone deciding
   it should, and new code is born non-compliant by default. Write the zone
   ...
   fall into without noticing is not an exemption, it is a hole. (This pairs
   with the retirement-trigger requirement above; together they are what
   stops a path zone from becoming permanent.)
